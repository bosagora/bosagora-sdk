...
import { InvalidAddressError, UnsupportedProtocolError } from "boa-sdk-common";
import { activeContractsList } from "boa-contracts-lib";
import { UnsupportedNetworkError } from "boa-sdk-common";
...
export class Context {
    ...
    get oldBOATokenContractAddress(): string | undefined {
        return this.state.oldBOATokenContractAddress;
    }

    get newBOATokenContractAddress(): string | undefined {
        return this.state.newBOATokenContractAddress;
    }

    get tokenSwapContractAddress(): string | undefined {
        return this.state.tokenSwapContractAddress;
    }
    ...
    setFull(contextParams: ContextParams): void {
        ...
        } else if (!contextParams.oldBOATokenContractAddress) {
            throw new Error("Missing old BOA token contract address");
        } else if (!contextParams.newBOATokenContractAddress) {
            throw new Error("Missing new BOA token contract address");
        } else if (!contextParams.tokenSwapContractAddress) {
            throw new Error("Missing token swap contract address");
        ...
        this.state = {
            ...
            oldBOATokenContractAddress: contextParams.oldBOATokenContractAddress,
            newBOATokenContractAddress: contextParams.newBOATokenContractAddress,
            tokenSwapContractAddress: contextParams.tokenSwapContractAddress,
            gasFeeEstimationFactor: Context.resolveGasFeeEstimationFactor(contextParams.gasFeeEstimationFactor)
        };
    }

    private convertNetworkName(chainName: string): string {
        switch (chainName) {
            case "homestead":
                return "mainnet";
            case "ethereum":
    ...
    set(contextParams: Partial<ContextParams>) {
        ...
        const networkName = this.convertNetworkName(this.state.network.name.toString());
        if (contextParams.oldBOATokenContractAddress) {
            this.state.oldBOATokenContractAddress = contextParams.oldBOATokenContractAddress;
        } else if (networkName in activeContractsList) {
            this.state.oldBOATokenContractAddress =
                activeContractsList[networkName as keyof typeof activeContractsList].OldBOAToken;
        }
        if (contextParams.newBOATokenContractAddress) {
            this.state.newBOATokenContractAddress = contextParams.newBOATokenContractAddress;
        } else if (networkName in activeContractsList) {
            this.state.newBOATokenContractAddress =
                activeContractsList[networkName as keyof typeof activeContractsList].NewBOAToken;
        }
        if (contextParams.tokenSwapContractAddress) {
            this.state.tokenSwapContractAddress = contextParams.tokenSwapContractAddress;
        } else if (networkName in activeContractsList) {
            this.state.tokenSwapContractAddress =
                activeContractsList[networkName as keyof typeof activeContractsList].TokenSwap;
        }
